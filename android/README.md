...
Capacitor's default WebView lives inside an Activity, so it disappears the moment the user leaves the app. To stay floating across the home screen and other apps, vibemoji adds a small native module:
...
2. Toggle "Allow display over other apps" for vibemoji, return.
...
- **Service killed in the background** → some OEMs aggressively kill foreground services. Disable battery optimization for vibemoji in Settings → Apps → vibemoji → Battery.
