...
- **Mobile:** Android only — via **Capacitor 8**. Renders the buddy as a true system overlay (drawn on top of every app, not just inside the vibebud activity) via a custom `OverlayService` + Capacitor `Overlay` plugin.
...
- `desktop/` — Electron shell that loads `core/`'s static export.
  ...
  - `preload.js` — exposes a small `window.vibebud` API (`setInteractive`, `setFocusable`, `setBounds`, `getCursorPoint`, `onSpawnBuddy`).
...
All host-bridge access goes through this module — no component reaches `window.vibebud`, `window.vibebudNative`, or `Capacitor.getPlatform()` directly. Adapters: `WebAdapter`, `ElectronAdapter`, `CapacitorAdapter`. Hooks: `usePlatform()`, `useLayout()` in `core/app/components/hooks/usePlatform.ts`.
...
# Desktop production installer (Windows NSIS → desktop/dist/vibebud-desktop-setup.exe)
...
- The web dev server runs on **port 3060**, not 3000. `desktop:dev` reads `VIBEBUD_DEV_URL` (set by the script to `http://localhost:3060`) and loads the `/buddy` route.
...
- API keys and buddy state live in browser `localStorage` (keys: `vibebud.buddies.v2`, `vibebud.provider.v1`, `vibebud.llmKey.<provider>.v1`, `vibebud.llmModel.<provider>.v1`). Use the tray menu's "Clear local settings" item to wipe.
...
- Keep platform-specific code out of `core/`. Detect Electron via `window.vibebud?.isElectron` and feature-detect rather than UA-sniffing.
- The renderer toggles mouse-event passthrough via `data-buddy-interactive` markers on interactive elements; a global `mousemove` handler in `Buddy.tsx` checks the element under the cursor and calls `vibebud.setInteractive()`. Don't break this — it's what makes the floating window not block the rest of the desktop.
